...
class OffsetDistProj(nn.Module):
    ...
    def forward(self, x):
        shape = x.size()
        x = F.softmax(x.reshape(*shape[:-1], 2, self.reg_max + 1), dim=-1)
        x = F.linear(x, self.project.type_as(x)).reshape(*shape[:-1], 2)
        return x


class ScaleDistProj(nn.Module):
    def __init__(
        self,
        reg_start,
        reg_end,
        ...
        super(ScaleDistProj, self).__init__()
    ...
    def forward(self, x):
        shape = x.size()
        x = F.softmax(x.reshape(*shape[:-1], 1, self.reg_max + 1), dim=-1)
        x = F.linear(x, self.project.type_as(x)).reshape(*shape[:-1], 1)
        return x


class LiteModelBasedHead(nn.Module):
    """Model-based detection head for task with pre-defined 3D model.
    ...
    Args:
        ...
        offset_reg_start (float): The minimal value of the offset between
            the feature map grid and the bbox center. Default: 0.0.
        offset_reg_end (float): The maximal value of the offset between
            the feature map grid and the bbox center. Default: 7.0.
        offset_reg_max (int): The maximal value of the discrete set to
            represent the offset. Default: 7.
        scale_reg_start (float): The minimal value of the square bbox scale.
            Default: 10.0.
        scale_reg_end (float): The maximal value of the square bbox scale.
            Default: 45.0.
        scale_reg_max (int): The maximal value of the discrete set to
            represent scale of the square bbox. Default: 15.
    ...
    def __init__(
        ...
        offset_reg_start=0.0,
        offset_reg_end=7.0,
        offset_reg_max=7,
        scale_reg_start=10.0,
        scale_reg_end=45.0,
        scale_reg_max=15,
        ...
        **kwargs
        ...
        super(LiteModelBasedHead, self).__init__()
        ...
        self.offset_reg_max = offset_reg_max,
        self.scale_reg_max = scale_reg_max,
        ...
        self.offset_dist_project = OffsetDistProj(
            reg_start=offset_reg_start,
            reg_end=offset_reg_end,
            reg_max=offset_reg_max
        )
        self.scale_dist_project = ScaleDistProj(
            reg_start=scale_reg_start,
            reg_end=scale_reg_end,
            reg_max=scale_reg_max
        )
            ...
            for w in range(self.width_branch_size):
                ...
                gfl_reg = nn.ModuleList(
                    [
                        nn.Conv2d(
                            self.feat_channels,
                            2 * (self.offset_reg_max + 1) + (self.scale_reg_max + 1),
                            1,
                            padding=0,
                        )
                        for _ in self.strides
    ...
    def loss(self, preds, gt_meta, aux_preds=None):
        ...
        for h in range(self.height_branch_size):
            for w in range(self.width_branch_size):
                ...
                cls_preds, offset_reg_preds, scale_reg_preds = branch_preds.split(
                    [self.num_classes, 2 * (self.offset_reg_max + 1), (self.scale_reg_max + 1)], dim=-1
                )
                ...
                offset_preds = self.offset_dist_project(offset_reg_preds) * center_priors[..., 2, None]
                scale_preds = self.scale_dist_project(scale_reg_preds)
                d0 = scale_preds[..., 0] - offset_preds[..., 0]
                d1 = scale_preds[..., 0] - offset_preds[..., 1]
                d2 = scale_preds[..., 0] + offset_preds[..., 0]
                d3 = scale_preds[..., 0] + offset_preds[..., 1]
                ...
                if aux_preds is not None:
                    ...
                    aux_cls_preds, aux_offset_reg_preds, aux_scale_reg_preds = aux_branch_preds.split(
                        [self.num_classes, 2 * (self.offset_reg_max + 1), (self.scale_reg_max + 1)], dim=-1
                    )
                    aux_offset_preds = self.offset_dist_project(aux_offset_reg_preds) * center_priors[..., 2, None]
                    aux_scale_preds = self.scale_dist_project(aux_scale_reg_preds)
                    d0 = aux_scale_preds[..., 0] - aux_offset_preds[..., 0]
                    d1 = aux_scale_preds[..., 0] - aux_offset_preds[..., 1]
                    d2 = aux_scale_preds[..., 0] + aux_offset_preds[..., 0]
                    d3 = aux_scale_preds[..., 0] + aux_offset_preds[..., 1]
                ...
                loss, loss_states = self._get_loss_from_assign(
                    cls_preds, offset_reg_preds, scale_reg_preds, decoded_bboxes, batch_assign_res
                )

                if aux_preds is not None:
                    aux_loss, aux_loss_states = self._get_loss_from_assign(
                        aux_cls_preds, aux_offset_reg_preds, aux_scale_reg_preds, aux_decoded_bboxes, batch_assign_res
                    )
                    loss = loss + aux_loss
    ...
    def _get_loss_from_assign(self, cls_preds, offset_reg_preds, scale_reg_preds, decoded_bboxes, assign):
        device = cls_preds.device
        labels, label_scores, bbox_targets, offset_targets, scale_targets, num_pos = assign
        ...
        offset_reg_preds = offset_reg_preds.reshape(-1, 2 * (self.offset_reg_max + 1))
        scale_reg_preds = scale_reg_preds.reshape(-1, self.scale_reg_max + 1)
        ...
        if len(pos_inds) > 0:
            ...
            scale_targets = torch.cat(scale_targets, dim=0)
            loss_dfl = self.loss_dfl(
                offset_reg_preds[pos_inds].reshape(-1, self.offset_reg_max + 1),
                offset_targets[pos_inds].reshape(-1),
                weight=weight_targets[:, None].expand(-1, 2).reshape(-1),
                avg_factor=2.0 * bbox_avg_factor,
            )
            loss_dfl += self.loss_dfl(
                scale_reg_preds[pos_inds].reshape(-1, self.scale_reg_max + 1),
                scale_targets[pos_inds].reshape(-1),
                weight=weight_targets[:, None].expand(-1, 1).reshape(-1),
                avg_factor=1.0 * bbox_avg_factor,
            )
        else:
            loss_bbox = offset_reg_preds.sum() * 0 + scale_reg_preds.sum() * 0
            loss_dfl = offset_reg_preds.sum() * 0 + scale_reg_preds.sum() * 0
    ...
    def target_assign_single_img(
        ...
        offset_targets = center_priors.new_zeros((num_priors, 2), dtype=torch.float)
        scale_targets = center_priors.new_zeros((num_priors, 1), dtype=torch.float)
        ...
        if num_gts == 0:
            ...
            return labels, label_scores, bbox_targets, offset_targets, scale_targets, 0
        ...
        if len(pos_inds) > 0:
            ...
            dist_targets = bbox2distance(center_priors[pos_inds, :2], pos_gt_keypoints)
            offset_targets[pos_inds, 0] = 0.5 * (dist_targets[..., 2] - dist_targets[..., 0])
            offset_targets[pos_inds, 0] /= center_priors[pos_inds, 2]
            offset_targets[pos_inds, 1] = 0.5 * (dist_targets[..., 3] - dist_targets[..., 1])
            offset_targets[pos_inds, 1] /= center_priors[pos_inds, 2]
            scale_targets[pos_inds, 0] = 0.25 * (dist_targets[..., 0] + dist_targets[..., 1] + dist_targets[..., 2] + dist_targets[..., 3])
    ...
    def get_bboxes(self, cls_preds, reg_preds, img_metas):
        ...
        Args:
            ...
            reg_preds (Tensor): Shape (num_imgs, num_points, 2 * (offset_regmax + 1) + (scale_regmax + 1)).
        ...
        for h in range(self.height_branch_size):
            for w in range(self.width_branch_size):
                ...
                offset_reg_preds, scale_reg_preds = branch_reg_preds.split(
                    [2 * (self.offset_reg_max + 1), (self.scale_reg_max + 1)], dim=-1
                )
                center_priors = torch.cat(mlvl_center_priors, dim=1)
                center_priors[..., 0] += (torch.ones_like(center_priors[..., 0]) * input_width * w)
                center_priors[..., 1] += (torch.ones_like(center_priors[..., 1]) * input_height * h)
